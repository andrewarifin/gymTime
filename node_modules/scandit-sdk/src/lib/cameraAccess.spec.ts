...
test.beforeEach(() => {
  (<any>CameraAccess.mediaStream) = undefined;
  (<any>window).MediaStreamTrack = undefined;
  (<any>navigator).enumerateDevices = undefined;
});
...
test.serial("getCameras (errors)", async t => {
  ...
  (<any>navigator).mediaDevices.getUserMedia = getUserMediaStub.rejects(new Error("Test error"));
  resetStubs();
...
test.serial("getCameras (navigator.mediaDevices.enumerateDevices)", async t => {
  fakeCompatibleBrowser();
  (<any>navigator.mediaDevices).enumerateDevices = enumerateDevicesStub.resolves([
    fakeCamera1,
    fakeCamera2,
    illegalFakeCamera1,
    fakeMicrophone
...
test.serial("getCameras (navigator.enumerateDevices)", async t => {
  fakeCompatibleBrowser();
  (<any>navigator).enumerateDevices = enumerateDevicesStub.resolves([
    fakeCamera1,
    fakeCamera2,
    illegalFakeCamera1,
    fakeMicrophone
...
test.serial("getCameras (internationalized label)", async t => {
  fakeCompatibleBrowser();
  (<any>navigator.mediaDevices).enumerateDevices = enumerateDevicesStub.resolves([fakeCamera3]);
  resetStubs();
  const cameras: Camera[] = await CameraAccess.getCameras();
  t.not(cameras, null);
...
test.serial("getCameras (no front/back label information)", async t => {
  fakeCompatibleBrowser();
  (<any>navigator.mediaDevices).enumerateDevices = enumerateDevicesStub.resolves([fakeCamera4, fakeCamera5]);
  resetStubs();
  const cameras: Camera[] = await CameraAccess.getCameras();
  t.not(cameras, null);
